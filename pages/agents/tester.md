...
This is an example `.agent.md` file for a BDD-focused testing agent. Its testing practices are drawn from [Defra software development standards](https://github.com/DEFRA/software-development-standards){:target="_blank"} (opens in new tab) — the single source of truth for Defra quality practices. Copy it into `.github/agents/tester.agent.md` in your repository.
...
describe('GET /applications/{id}', () => {
  let server
  beforeAll(async () => { server = await createServer(); await server.initialize() })
  afterAll(async () => { await server.stop() })

  it('should return 200 for a valid application', async () => {
    const response = await server.inject({ method: 'GET', url: '/applications/123' })
    expect(response.statusCode).toBe(200)
  })

  it('should return 404 when application does not exist', async () => {
    const response = await server.inject({ method: 'GET', url: '/applications/does-not-exist' })
